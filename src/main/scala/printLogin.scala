...
class Login(basedir: String,schema: scala.collection.mutable.HashMap[String, List[String]], authUserField: String, authPWField: String) {
	...
	def print() {	
		...
		write(writer,tab+tab+tab+"@helper.inputText(form(\""+authUserField+"\"), '_label -> \"Name\")")
		write(writer,tab+tab+tab+"@helper.inputPassword(form(\""+authPWField+"\"), '_label -> \"Password\")")
