...
class ListActivity(objectName: String, fields: List[String], primaryKey: Option[String], basedir: String, url: String ) extends scalaMVCFile(objectName, fields, primaryKey, basedir, url)   {

	def print() = {
		val fileName = objectName.capitalize+"ListActivity.java"
		...
		write(writer,"")
		write(writer,"public class "+capitalizedObjectName+"ListActivity extends Activity {")
		for ( f <- fields ) {
...
// fix numbers after decimal point issue with doubles
		...
		write(writer,tab+tab+tab+tab+tab+tab+"formIntent = new Intent(getApplicationContext(), "+capitalizedObjectName+"FormActivity.class);")
